...
- Manual installs (npm/go/curl): edit `.chezmoiscripts/run_onchange_after_30_manual.sh.tmpl` and add idempotent commands. Go installs are routed to `~/.local/bin` via `GOBIN` so they land on PATH.
